...
const page = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get(`http://prespottunes.daw.inspedralbes.cat:8000/public/api/users/${User.id}/followers`, {
                    headers: {
        ...
        const fetchData2 = async () => {
            try {
                ...
                const response = await axios.get(`https://spottunes.daw.inspedralbes.cat:8000/public/api/users/${User.id}/followed`, {
                    headers: {
                        'Authorization': `Bearer ${Token}`
                    }
                });
    ...
    return (
        <>
            <main className='bg-background flex flex-col gap-10 min-h-screen'>
                ...
                <section className='flex flex-col gap-4'>
                    <div className='flex justify-center gap-8'>
                        ...
                        <button 
                            className={selectedSection === 'Posts' ? 'border-b-2 border-b-white' : 'opacity-60'} 
                            onClick={() => setSelectedSection('Posts')}
                        >
                            Posts
                        </button>
                        <button 
                            className={selectedSection === 'Eventos' ? 'border-b-2 border-b-white' : 'opacity-60'} 
                            onClick={() => setSelectedSection('Eventos')}
                        >
                            Eventos
                        </button>
                        <button 
                            className={selectedSection === 'Gustos' ? 'border-b-2 border-b-white' : 'opacity-60'} 
                            onClick={() => setSelectedSection('Gustos')}
                        >
                            Gustos
                        </button>
